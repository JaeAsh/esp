...
if QuantomUniversalAimbotLoadingTime == true then
    Notify("Quatom Solutions", "Script Loaded Already", "", 3)
    return 
end
    ...
    if workspace and workspace:FindFirstChild"Teams" then 
        if getgenv().TeamCheck == true then
            if #workspace.Teams:GetChildren() == 0 then 
                ...
                SeparateNotify("Quatom Solutions", "TeamCheck set to: "..tostring(getgenv().TeamCheck).." because there are no teams!", "", 3)
            end
        end
    end
end
        ...
        if p ~= Client then 
            if p.Character and p.Character:FindFirstChild(getgenv().AimPart) then 
                if getgenv().TeamCheck == true and p.Team ~= Client.Team then 
                    ...
                    local MPos = Vec2(Mouse.X, Mouse.Y)
...
--                    Notify("Quatom Solutions", "Aimlock Target: "..tostring(AimlockTarget), "", 3)
--                end
--            end)
--       end
--        if Key.KeyCode == Enum.KeyCode[AimlockToggleKey] then 
--            Aimlock = not Aimlock
--           Notify("Quatom Solutions", "Aimlock: "..tostring(Aimlock), "", 3)
--       end
    end
end)
...
SeparateNotify("Quatom Solutions", "Universal Aimbot loaded in: "..string.format("%.7f", tostring(tick() - QuantomUniversalAimbotLoadingTime)), "", 3)
